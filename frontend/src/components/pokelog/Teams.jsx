...
import WhoGoesFirst from './battle/WhoGoesFirst'
...
function Teams({ teams, onRefresh }) {
    ...
    const [pendingTeams, setPendingTeams] = useState({ team1Id: null, team2Id: null })
    ...
    const handleStartBattle = (team1Id, team2Id) => {
        setPendingTeams({ team1Id, team2Id })
        setBattleSetupOpen(false)
        setWhoGoesFirstOpen(true)
    }

    const handleGoesFirst = async (choice) => {
        setWhoGoesFirstOpen(false)
        ...
        try {
            ...
            alert(err.response?.data?.detail || 'Failed to start battle')
        } finally {
    ...
    const handleCloseBattle = () => {
        setArenaOpen(false)
        setBattleState(null)
        setPendingTeams({ team1Id: null, team2Id: null })
    }
    ...
    return (
        <div>
            ...
            {/* Battle modals */}
            <BattleSetup
                open={battleSetupOpen}
                onClose={() => setBattleSetupOpen(false)}
                teams={teams}
                onStartBattle={handleStartBattle}
            />

            {whoGoesFirstOpen && pendingTeams.team1Id && (
                <WhoGoesFirst
                    open={whoGoesFirstOpen}
                    onClose={() => setWhoGoesFirstOpen(false)}
                    team1Name={teams.find(t => t.id === pendingTeams.team1Id)?.team_name || ''}
                    team2Name={teams.find(t => t.id === pendingTeams.team2Id)?.team_name || ''}
                    onChoose={handleGoesFirst}
                />
            )}

            {arenaOpen && battleState && (
                <BattleArena
                    open={arenaOpen}
