...
function Teams({ teams, onRefresh }) {
  ...
  const handleDeleteTeam = async (teamId) => {
    ...
    try {
      await api.delete(`/pokemon_team/${teamId}`)
      onRefresh()
    } catch (err) {
      console.error(err)
    }
  ...
  const handleRemoveMember = async (teamId, slot) => {
    if (!window.confirm(`Remove Pokémon from slot ${slot}?`)) return
    try {
      await api.delete(`/pokemon_team/${teamId}/members/${slot}`)
      onRefresh()
    } catch (err) {
      console.error(err)
    }
