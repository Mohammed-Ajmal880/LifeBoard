  ...
  const handleMove = async (move) => {
    ...
    try {
      ...
      setState(prev => ({
        ...
        active1: data.player_team.findIndex(p => !p.fainted && p === data.player_pokemon)  !== -1
          ? data.player_team.indexOf(data.player_pokemon)
          : prev.active1,
        active2: data.opponent_team.indexOf(data.opponent_pokemon) !== -1
          ? data.opponent_team.indexOf(data.opponent_pokemon)
          : prev.active2,
      }))

      // Sync active indices properly
      setState({
        ...state,
        team1:   data.player_team,
        team2:   data.opponent_team,
